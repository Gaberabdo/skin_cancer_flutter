...
class IsolateInference {
  ...
  static void entryPoint(SendPort sendPort) async {
    ...
    await for (final InferenceModel isolateModel in port) {
      ...
      //final output = [List<int>.filled(isolateModel.outputShape[1], 0)];  //uint8 in tflite model
      final output = [List<double>.filled(isolateModel.outputShape[1], 0.0)]; //float32 in tflite model
      // // Run inference
      ...
      //int maxScore = result.reduce((a, b) => a + b); //uint8 in tflite model
      //double maxScore = result.reduce((a, b) => a + b).toDouble(); //float32 in tflite model
